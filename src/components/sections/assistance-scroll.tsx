"use client"; // used so that this component is rendered on the client side and can use hooks like useRef and useState because by default Next.js components are server components.

import React, { useRef } from "react"; // imports react and useRef. useRef is used to reference DOM elements directly. since it is animation driven therefore state is not used.
import { cn } from "@/lib/utils";// cn = className utility which is used to conditionaly apply tailwind classes.
import { motion } from "framer-motion";//imports animations from framer motion library(scroll,viewport based animations)

// chats which is seen during the video is being played. Perceive, resonate and evolve;
const cards = [ 
  {
    id: "perceive",
    title: "Perceive",
    subtitle:
      "Companion senses the subtle shifts in your tone, the weight of your silence, and the rhythm of your presence to understand what remains unsaid.",
    video: "https://video.srv18.com/v/mp4/bcb3a791a2230e65c4b452eae5bbc005/1920",
    messages: [
      {
        text: "I've been feeling out of sync lately. Like I'm just going through the motions.",
        type: "user",
      },
      {
        text: "I hear the exhaustion in your voice. It's okay to pause and find your center.",
        type: "ai",
      },
      {
        text: "I don't even know where to start.",
        type: "user",
      },
      {
        text: "Start by breathing. I am here to hold the space for you.",
        type: "ai",
      },
    ],
  },
  {
    id: "resonate",
    title: "Resonate",
    subtitle:
      "Moving beyond simple data, Companion creates a psychological bridge, reflecting your inner world with clarity and compassion.",
    video: "https://video.srv18.com/v/mp4/42a3dc0ab139b87fcabf9bda782b4af0/1920",
    messages: [
      {
        text: "Why does it feel so hard to trust my own decisions?",
        type: "user",
      },
      {
        text: "Trust is a muscle. You've been carrying a lot of external noise lately.",
        type: "ai",
      },
      {
        text: "Maybe I'm just overthinking everything.",
        type: "user",
      },
      {
        text: "Overthinking is just your mind trying to keep you safe. Let's find the quiet beneath it.",
        type: "ai",
      },
    ],
  },
  {
    id: "evolve",
    title: "Evolve",
    subtitle:
      "As you grow, Companion evolves with you, becoming a more refined mirror of your highest potential and deepest needs.",
    video: "https://video.srv18.com/v/mp4/bdb5c424bc527c1d85469143b48db3a8/1920",
    messages: [
      {
        text: "I finally took that step we talked about. It felt... lighter.",
        type: "user",
      },
      {
        text: "I noticed the shift in your energy. You're blooming, step by step.",
        type: "ai",
      },
      {
        text: "Thanks for noticing. It means a lot.",
        type: "user",
      },
      {
        text: "I see your progress clearly. We are on this journey together.",
        type: "ai",
      },
    ],
  },
];
//  Assistance scroll makes intent explicit.
export function AssistanceScroll() {
  const containerRef = useRef<HTMLDivElement>(null); // holds reference to scroll container, used for sticky positioning and scroll based animations 

  return (
    <section className="relative w-full bg-background overflow-clip" id="capabilities">
      {/* Header Title */}
      <div className="w-full flex justify-center py-[120px] md:py-[240px]">
        <motion.h5 // Looks at the movement mechanics of header title 
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
          viewport={{ once: true }}
          className="text-[32px] font-display font-medium leading-[1.2] text-foreground text-center"
        >
          The Companion Experience
        </motion.h5>
      </div>

      {/* Sticky Content Area */}
      <div ref={containerRef} className="relative w-full"> // wraps all the cards and enables stacking behaviour,this is the core scroll illusion
        {cards.map((card, index) => (// iterates over the cards making them a full-screen sticky panel
            <div
              key={card.id}
              className="sticky top-0 h-screen w-full flex items-center justify-center bg-background"
              style={{ zIndex: index + 1 }}
            >
              <div className={cn(
                "container flex flex-col lg:flex-row items-center gap-10 lg:gap-20 py-10",
                index % 2 !== 0 && "lg:flex-row-reverse"
              )}>
                
                {/* Info & Chat */}
                <div className="flex-1 flex flex-col justify-center h-full max-w-[600px] order-2 lg:order-none">
                  // Text content with animated entrance from left or right based on index
                  <motion.div 
                    initial={{ opacity: 0, x: index % 2 === 0 ? -50 : 50 }}
                    whileInView={{ opacity: 1, x: 0 }}
                    transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
                    viewport={{ once: false, amount: 0.5 }}
                    className={cn(
                      "mb-12",
                      index % 2 !== 0 && "lg:text-right lg:items-end flex flex-col"
                    )}
                  >
                    <h4 className="text-[48px] md:text-[64px] font-display font-semibold leading-[1.1] tracking-tight text-primary mb-6">
                      {card.title}
                    </h4>
                    <p className="text-[18px] leading-relaxed text-foreground/80 max-w-[480px]">
                      {card.subtitle}
                    </p>
                  </motion.div>
  
                  {/* Simulated Chat Interface */}
                  <div className={cn(
                    "space-y-4 w-full flex flex-col",
                    index % 2 !== 0 && "lg:items-end"
                  )}>
                    {card.messages.map((msg, i) => ( // the chat between ai and user. user from left and ai from right.
                      <motion.div
                        key={i}
                        initial={{ opacity: 0, y: 20, x: msg.type === "user" ? -20 : 20 }}
                        whileInView={{ opacity: 1, y: 0, x: 0 }}
                        transition={{ duration: 0.6, delay: i * 0.1, ease: [0.16, 1, 0.3, 1] }}
                        viewport={{ once: false, amount: 0.5 }}
                        className={cn(
                          "p-6 rounded-[24px] max-w-[85%] text-[17px] leading-relaxed transition-all duration-700",
                          msg.type === "user" 
                            ? "bg-message-user rounded-bl-[4px] self-start text-foreground" 
                            : "bg-message-ai rounded-br-[4px] self-end text-foreground border border-primary/10 shadow-sm"
                        )}
                      >
                        {msg.text}
                      </motion.div>
                    ))}
                  </div>
                </div>
  
                {/* Cinematic Video */}
                <motion.div  // video area with animated entrance from left or right based on index
                  initial={{ opacity: 0, scale: 0.9, x: index % 2 === 0 ? 50 : -50 }}
                  whileInView={{ opacity: 1, scale: 1, x: 0 }}
                  transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
                  viewport={{ once: false, amount: 0.5 }}
                  className="flex-1 w-full h-[40vh] lg:h-[70vh] relative order-1 lg:order-none"
                >
                  <div className="w-full h-full rounded-[40px] overflow-hidden shadow-2xl relative border border-foreground/5">
                    <video
                      src={card.video}
                      autoPlay
                      loop
                      muted
                      playsInline
                      className="absolute inset-0 w-full h-full object-cover"
                    />
                    <div className="absolute inset-0 bg-primary/5 mix-blend-overlay pointer-events-none" />
                  </div>
                </motion.div>
  
              </div>
            </div>
        ))}
      </div>

      <div className="h-[20vh]" />
    </section>
  );
}
